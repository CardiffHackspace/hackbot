...
var HackBot = function Constructor(settings) {
  ...
  this.dbPath = settings.dbPath || path.resolve(process.cwd(), 'data', 'hackbot.db');
  this.user = null;
  this.db = null;
  this.defaultChannelName = 'hackbot-development';
  this.lightSensor = new GPIO(17, 'in', 'both');
}
...
HackBot.prototype._onLightChange = function(err, state) {
  console.log('light sensor state changed to ' + state);
};
